...
const SlotMachine = () => {
  ...
  const height = 210;
  const reels = [
    ["/bnb2.webp", "/tracexnew.webp", "/bitcoin.png"],
    ["/bnb2.webp", "/tracexnew.webp", "/bitcoin.png"],
    ["/bnb2.webp", "/tracexnew.webp", "/bitcoin.png"],
  ];
    ...
    if (result[0] === result[1] && result[1] === result[2]) {
      ...
      setMsg(
       ...
       <span className="text-primary">
        Congratulations! You won <span className="font-semibold text-accent text-nowrap">{formatNumber(award)}</span> $TRACEX, Click on Claim now!
        </span> 
        </>
      );
    } else {
      ...
      <div className='w-full flex justify-between items-center pb-2 px-4'>
        ...
        <div className='flex items-center'>
          <span className='flex items-center mt-[-1px]'>
            <img src='/tracexnew.webp' className='w-[25px]' alt='engagecoin'/>
          </span>
          <span className='text-[#fff] font-semibold text-[16px]'>
            <span className='pl-[4px]'>{formatNumber(points)} <span className='text-secondary'>$TRACEX</span></span>
          </span>
        </div>
...
<div ref={modalRef} className={`w-full bg-divider shadowtop rounded-tl-[40px] rounded-tr-[40px] relative flex flex-col ease-in duration-300 transition-all justify-center`}>
 ...
 Your next free Spin starts in
    ...
    <div className="w-full flex justify-between items-center gap-2 px-4">
        ...
        <span className="text-nowrap">Good Luck 💪</span>
        <div className="w-[40%] h-[2px] bg-cards2"></div>
