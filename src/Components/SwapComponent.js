...
const SwapComponent = ({openSwapModal, setOpenSwapModal}) => {
    ...
    const [selectedAssetFrom, setSelectedAssetFrom] = useState(walletAssets.find(asset => asset.symbol === '$TRACEX'));
    const [selectedAssetTo, setSelectedAssetTo] = useState(walletAssets.find(asset => asset.symbol === 'USDT'));

    const handleMaxClick = (fromBalance, fromSymbol, toSymbol, fromPrice, toPrice) => {
      // Calculate the max amount that can be swapped
      const maxAmount = fromBalance;  // You can calculate the max based on conditions like fees, etc.
    
      // Set the swapAmount to maxAmount
      setSwapAmount(maxAmount);
    
      // Now calculate the swap value for the 'To' asset
      const maxSwapValue = (maxAmount * fromPrice) / toPrice;
    
      // Update the calculatedSwapValue
      setCalculatedSwapValue(maxSwapValue);
    };
    ...
    const handleSwap = () => {
      ...
      const confirmSwap = async () => {
        ...
        const updatedAssets = walletAssets.map(asset => {
            if (asset.symbol === selectedAssetFrom.symbol) {
                if (selectedAssetFrom.symbol === '$TRACEX') {
        ...
        try {
          await updateDoc(userRef, {
            ...
            ...(selectedAssetFrom.symbol === '$TRACEX' && { balance: newBalance }),
          ...
          setSwaps(prevSwaps => [...prevSwaps, swapHistory]);
            ...
            setTimeout(() => {
                ...
                setSwapAmount('');
                setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === '$TRACEX'));
      ...
      const handleAssetSelection = (asset, type) => {
        if (type === 'from') {
          ...
          if (asset.symbol === '$TRACEX') {
            const filteredAssets = walletAssets.filter(asset => asset.symbol !== '$TRACEX');
      ...
      ? walletAssets.filter(asset => asset.symbol !== '$TRACEX') 
      : walletAssets;
      ...
      const cancelSwap = () => {
        ...
        setSwapAmount('');
        setCalculatedSwapValue(0);
        setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === '$TRACEX')); // Reset to default 'from' asset
...
<div className="flex justify-between items-center">
  ...
  {selectedAssetFrom && (
              ...
              onClick={() => 
                handleMaxClick(
                  selectedAssetFrom.balance, 
                  selectedAssetFrom.symbol, 
                  selectedAssetTo.symbol, 
                  selectedAssetFrom.price, 
                  selectedAssetTo.price
                )
              }
