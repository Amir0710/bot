...
const PphInfo = ({info, setInfo}) => {
  ...
  const handleClick = () => {
    if (location.pathname === '/mine') {
      setInfo(false); // Close the modal if the user is already on /mine
    }
  };
  ...
  return (
        ...
        <div ref={infoRef} className={`w-full bg-divider shadowtop rounded-tl-[40px] rounded-tr-[40px] relative flex flex-col ease-in duration-300 transition-all justify-center`}>
         ...
         <PiRocketThin className="w-[34px] h-[34px] text-[#f5bb5f]" />
        </div>
            ...
            Upgrade your mining cards to increase your profit per hour, even while offline, for up to 3 hours of continued earnings.
          </p>
            </div>
