...
export const UserProvider = ({ children }) => {
  ...
  const assets = [
    {symbol: '$TRACEX', name: 'Tracex', balance: balance, icon: '/tracexnew.webp',price: 0.0002},
    ...
    if (telegramUser) {
      ...
      try {
        ...
        const userData = {
          ...
          selectedExchange: {id: 'binance', icon: '/binance.webp', name: 'Binance'},
          energy: 500,
        ...
        setSelectedExchange({id: 'binance', name: 'Binance', icon: '/binance.webp'});
  ...
  useEffect(() => {
    ...
    // Find the current '$TRACEX' token in walletAssets
    const maxToken = walletAssets.find(asset => asset.symbol === '$TRACEX');
  
    // Check if $TRACEX exists and if its balance is different from the newBalance
    if (maxToken && maxToken.balance !== newBalance) {
      // Update the balance for the '$TRACEX' token
      setWalletAssets(prevAssets =>
        prevAssets.map(asset =>
          asset.symbol === '$TRACEX' ? { ...asset, balance: newBalance } : asset
        )
      );
    }
