...
const ReferralRewards = () => {
        ...
        setModalMessage(
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <h3 className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2">
              <span className="text-accent">+{formatNumber(reward.bonusAward)}</span> $TRACEX CLAIMED
            </h3>
  ...
  const formatNumberCliam = (num) => {
    if (num < 100000) {
      ...
      return (num / 100000000).toFixed(3).replace(".", ".") + " T";
    }
  };
...
<button onClick={() => setShowLevel(true)} className='w-[55%] flex space-x-1 items-center'>
    ...
    <div className='w-full flex items-center justify-between'>
      ...
      <div onClick={() => handleMenu(2)} className={`${activeIndex === 2 ? 'bg-cards3 text-[#ebebeb]' : ''} barTitle rounded-[6px] text-[#c6c6c6] py-[10px] px-3 w-[45%] space-x-2 font-semibold text-[15px] flex justify-center text-center items-center`}>
        <GiRank3  size={16} className="" />  <span>
          Rank
        </span>
      </div>

      <div onClick={() => handleMenu(3)} className={`${activeIndex === 3 ? 'bg-cards3 text-[#ebebeb]' : ''} barTitle rounded-[6px] text-[#c6c6c6] py-[10px] px-3 w-[45%] space-x-2 font-semibold text-[15px] flex justify-center text-center items-center`}>
        <PiRankingThin  size={16} className="" />  <span>
          Leaderboard
        </span>
      </div>
        ...
        .map((reward) => {
          ...
          return (
            <div key={reward.title} className="bg-cards w-full rounded-[15px] p-[14px] flex flex-wrap justify-between items-center">
              ...
              <div className=''>
                {reward.title === "Invite 1 friend" && (
                    <img src='/frens1.webp' alt="bonuses" className='w-[50px]'/>
                )}
                {reward.title === "Invite 5 friends" && (
                    <img src='/frens5.webp' alt="bonuses" className='w-[50px]'/>
                )}
                {reward.title === "Invite 10 friends" && (
                    <img src='/frens10.webp' alt="bonuses" className='w-[50px]'/>
                )}
                {reward.title === "Invite 25 friends" && (
                    <img src='/frens252.webp' alt="bonuses" className='w-[50px]'/>
                )}
                {reward.title === "Invite 50 friends" && (
                    <img src='/frens50.webp' alt="bonuses" className='w-[50px]'/>
                )}
            </div>
                <div className="flex flex-col space-y-1">
                  <span className="font-semibold text-[15px]">{reward.title}</span>
                  ...
                  <span className="">
                    ...
                    <span className="font-medium text-[14px]">{formatNumberCliam(reward.bonusAward)}</span>
                  </div>
                </div>
              </div>
