...
const DailyCheckIn = () => {
  const { id, fullName,level, tapBalance, showStartOverModal, setShowStartOverModal, showClaimModal, setShowClaimModal, refBonus, setLastCheckIn, setError, error, checkInDays, setCheckInDays, balance, setBalance, selectedCharacter } = useUser();
    ...
    return bonusPoints.map((points, index) => {
      ...
      return (
        ...
        >
          ...
          <img src="/tracexnew.webp" alt='coin' className='w-[25px]' />
...
<div id="refer" className='w-full flex flex-col h-[80vh] overflow-y-auto pb-[180px] pt-3 px-4'>
...
<div className={`w-full flex-col pb-4 flex items-center justify-center text-center pt-4`}>
    ...
    <FaCalendarAlt size={50} className='text-accent'/>
            ...
            Log in daily to collect your $TRACEX token rewards!
            </p>
          </div>
            ...
            <button className='w-[23%] space-y-1 bg-cards flex flex-col items-center justify-center relative h-[80px] rounded-[8px] cursor-pointer select-none'>
                <h2 className='text-[10px]'>
                   <PiLockKeyFill size={10} className='text-[#C7C7C7]'/>
                    ...
                    <img src="/tracexnew.webp" alt='fsd' className='w-[25px]'/>
            ...
            >
              <div className="w-full flex justify-center flex-col items-center space-y-3">
                <div className="w-full items-center justify-center flex flex-col space-y-2">
                  ...
                  <p className='font-medium'>Daily Login bonus claimed</p>
                </div>
                <h3 className="font-medium text-[24px] text-[#ffffff] pb-2">
                  {streak === true ? (
<>
<span className='text-accent'>+{formatNumberCliam(bonusPoints[14])}</span> $TRACEX
</>
                  ) : (
<>
<span className='text-accent'>+{formatNumberCliam(bonusPoints[checkInDays.length - 1])}</span> $TRACEX
</>
         
         )}
                </h3>
                <p className="pb-6 text-primary text-[15px] w-full text-center">
                  Daily Login bonus claimed! <br /> <br />
                  Come back tomorrow to claim another Login bonus!
                </p>
...
<div className="w-full text-[#5a3d00] flex items-center space-x-2 px-4 bg-[#f5bb5f] h-[50px] rounded-[8px]">
