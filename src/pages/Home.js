...
const Home = () => {
  ...
  useEffect(() => {
    ...
    // Only restrict access if not on mobile, Telegram, or specific dashboard route
    if (
      !isMobile && !isDashboardRoute && 
      restrictedBrowsers.includes(browserName) && 
  ...
  if (restrictAccess) {
    window.location.href = 'https://t.me/tracextapbot';
    return null;
  }
              ...
              <UserProvider>
                <AnimatePresence mode="wait">
                  <>
                    ...
                    <div
                      id="footermain"
                      className={`${loading ? 'invisible' : 'visible'} bg-[#171717] z-30 flex flex-col rounded-tr-[20px] rounded-tl-[20px] fixed bottom-0 ${isIOS ? 'pb-6' : 'pb-3'} left-0 right-0 justify-center items-center px-3`}
                    >
                      <Footer />
