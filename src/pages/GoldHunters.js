...
const GoldHunters = () => {
  ...
  //       const fetchedProfitHour = userData.profitHour; // Get profitHour directly
  //       console.log('Fetched profitHour:', fetchedProfitHour);
  //       setProfitHour(fetchedProfitHour); // Update state
  //     } else {
...
// const awardPoints = () => {
//     console.log('Profit Per Hour:', profitHour); // Log profitHour
...
//         console.log('Points per second based on profitHour:', pointsPerSecond);
//         console.log('Points to award:', pointsToAward);

//         // Only award points if profitHour is greater than 0
//         if (profitHour > 0) {
//             setExchangePoints(prevExchangePoints => prevExchangePoints + pointsToAward);
//         } else {
//             console.log('Profit hour is 0, not awarding points.');
//         }
//     }
// };

const awardPoints = () => {
  const profitperhour = profitHour;
  console.log('PPH', profitperhour);
  const savedTime = localStorage.getItem('currentTime');
  if (savedTime) {
    const now = new Date();
    const savedDate = new Date(savedTime);
...
useEffect(() => {
  // Set interval to award points every hour
  setInterval(() => {
    awardPoints();
  }, 3600000); // 3600000 ms = 1 hour
}, []); // Empty dependency array ensures this runs only once on mount

// Fetch profitHour when component mounts or id changes
// useEffect(() => {
//     fetchProfitHour();
// }, [id]);

// Award points whenever profitHour changes
useEffect(() => {
    if (profitHour > 0) {
        awardPoints(); // Call awardPoints only if profitHour is greater than 0
    }
}, [profitHour]);
        ...
        <div className='flex w-[60%] bg-[#5c5c5c52] border-[1px] border-[#434343] h-[40px] mb-[-4px] py-3 px-3 rounded-full items-center'>
            ...
            <div className='flex flex-1 flex-col space-y-1 items-center justify-center'>
              ...
              <span className='flex items-center justify-center'>
                <img src='/tracexnew.webp' alt='ppf' className='w-[20px]'/>
                  </span>
                  {/* {profitHour !== null ? (
...
<span className='text-[16px]'>
  ...
  <span className='text-[16px]'>
    <RxRocket  className='tesxt-[#bcbcbc] text-btn4' />
  </span>
  <span className='text-nowrap'>
    Boosters
  </span> 
    ...
    <PiArrowRightBold size={16} className='text-secondary'/>
      ...
      <h1 className='flex w-full justify-center items-center space-x-1 pb-2'>
         <span className='text-[#fff] text-[32px] font-bold'>
           <span className='pl-[2px]'>{formatNumber(balance + refBonus)} <span className='text-btn4'>$TRACEX</span></span>
        </span>
      </h1>
      <div>
    </div>
...
{/* <div className="w-full" style={{ position: 'relative' }}>
  <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 10 }}>
    ...
    <div style={{ textAlign: 'center', fontSize: '10px', color: '#f5bb5f', fontWeight: 'bold' }}> 
      Lucky Spin
    </div>
  ...
  <div class="after"></div>
      ...
      {energy === 0 && points === 0 ? (
          ...
          <span className='text-[#fff] font-semibold text-[24px]'>
            <span className='pl-[2px]'>{points} <span className='text-btn4'>$TRACEX</span></span>
          </span>
          <button 
            onClick={handleClaim} 
            disabled={points === 0} 
      ...
      >
        ...
        <div className={`${
          ...
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <h3 className="font-medium text-[24px] text-[#ffffff] pt-2 pb-2">
              <span className='text-accent'>+{points}</span> $TRACEX
            </h3>
            <p className="pb-6 text-[#bfbfbf] text-[15px] w-full text-center">
              Keep grinding! something huge is coming! Get more $TRACEX now! 
            </p>
        ...
        {selectedExchange.id === 'selectex' || exchangePoints < 1 ? (
          <>
          </>
        ) : (
        ...
        <div ref={exchangeRef} className={`w-full bg-divider shadowtop rounded-tl-[40px] rounded-tr-[40px] relative flex flex-col ease-in duration-300 transition-all justify-center`}>
         ...
         <div className='w-full bg-cards rounded-[16px] relative px-4 flex flex-col justify-center items-center'>
            ...
            <div className='w-full items-center flex pt-1 justify-center space-x-[6px]'>
              <img src='/tracexnew.webp' alt='coin' className='w-[36px]'/>
          ...
          The exchange has started working for you
            ...
            >
            <span className="text-[#5a3d00]">Thank you, {selectedExchange.name}</span>
            <Icon icon="tabler:heart-star" size={20} className="mt-[2px] pl-1 pump" />
            </button>
          </div>
          </div>
          </div>
...
<div className={`w-full bg-divider shadowtop rounded-tl-[40px] rounded-tr-[40px] relative flex flex-col ease-in duration-300 transition-all justify-center`}>
  ...
  Discover Tracex Bot <br/> Earn $TRACEX, Convert to USDT, & Claim Airdrops!
</h1>
